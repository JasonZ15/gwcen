...
function init_tinymce(editor_id, element) {
	...
	switch_btn.bind('click', function(){
		...
		if(button.is('.switch-tmce')){
			...
			var $tmp_val = jQuery('#'+editor_id).val();
			$tmp_val = $tmp_val.replace(/\\/g,'');
			window.tinyMCE.get(editor_id).setContent(window.switchEditors.wpautop($tmp_val), {format:'raw'});
		}
		else{
			editor_wrapper.removeClass('tmce-active').addClass('html-active');
			window.tinyMCE.execCommand("mceRemoveControl", true, editor_id);
		}
	}).trigger('click');
}
...
function add_blox_element($container, $pos){
	...
	$modal.append('<div class="blox_elements_toolbar"> \
						<span class="element_filter"> \
							...
							<a href="javascript:;" onclick="jQuery(\'#blox_iso_item_search\').val(\'\'); isotopeSearch(false); return false;"><i class="icon-remove"></i></a> \
						</span> \
					</div>');
						...
						jQuery.post( ajaxurl, {'action':'blox_template_save', 'title':jQuery('#blox_save_template_name').val(), 'content':$content.html()}, function(data){
							if( data!='-1' ){
								...
								for(var i=0; i<$json.length; i++){
									html += '<span><a href="javascript: blox_load_template(&quot;'+$json[i].id+'&quot;);" data-template="'+$json[i].id+'">'+$json[i].title+'</a><i class="icon-remove" onclick="blox_remove_template(jQuery(this));"></i></span>';
								}
...
jQuery(function(){
	...
	jQuery('#publish').click(function(){
		if( jQuery("#blox_contentbuilder").css('display') == 'block' ){
			$content = jQuery('#blox_preview').clone();
			...
			//tinyMCE.activeEditor.setContent($content.html());
			//tinyMCE.get('content').setContent($content.html());
			...
			window.tinyMCE.get('content').setContent( window.tinyMCE.get('content').getContent() );
			window.tinyMCE.get('content').execCommand('mceRepaint');
		}
	});
...
function show_blox_form_handler($form, callback, params){
    ...
    jQuery('#blox_popup_window').find('.blox_popup_button_close').unbind('click')
		.click(function(){
			...
			window.tinyMCE.execCommand('mceRemoveControl', false, 'blox_tinymce_editor');
