...
<div id="order_review">
	...
	<table class="shop_table">
		<thead>
			<tr>
				<th class="product-name"><?php _e( 'Product', 'themeton' ); ?></th>
				<th class="product-total"><?php _e( 'Total', 'themeton' ); ?></th>
			</tr>
		</thead>
		<tfoot>
			<tr class="cart-subtotal">
				<th><?php _e( 'Cart Subtotal', 'themeton' ); ?></th>
			...
			<tr class="discount">
				<th><?php _e( 'Cart Discount', 'themeton' ); ?></th>
			...
			<tr class="discount">
				<th><?php _e( 'Order Discount', 'themeton' ); ?></th>
					...
					<?php
						...
						if ( $woocommerce->cart->tax_display_cart == 'incl' ) {
							...
							if ( ! empty( $tax_string_array ) ) {
								?><small class="includes_tax"><?php printf( __( '(Includes %s)', 'themeton' ), implode( ', ', $tax_string_array ) ); ?></small><?php
							}
						}
					?>
				</td>
	...
	<div id="payment">
			...
			<?php
				...
				if ( ! empty( $available_gateways ) ) {
					...
					if ( ! $woocommerce->customer->get_country() )
						echo '<p>' . __( 'Please fill in your details above to see available payment methods.', 'themeton' ) . '</p>';
					else
						echo '<p>' . __( 'Sorry, it seems that there are no available payment methods for your state. Please contact us if you require assistance or wish to make alternate arrangements.', 'themeton' ) . '</p>';
		...
		<div class="form-row place-order">
			...
			<noscript><?php _e( 'Since your browser does not support JavaScript, or it is disabled, please ensure you click the <em>Update Totals</em> button before placing your order. You may be charged more than the amount stated above if you fail to do so.', 'themeton' ); ?><br/><input type="submit" class="button alt" name="woocommerce_checkout_update_totals" value="<?php _e( 'Update totals', 'themeton' ); ?>" /></noscript>
			...
			<?php
			$order_button_text = apply_filters('woocommerce_order_button_text', __( 'Place order', 'themeton' ));
			...
			<p class="form-row terms">
				<label for="terms" class="checkbox"><?php _e( 'I have read and accept the', 'themeton' ); ?> <a href="<?php echo esc_url( get_permalink(woocommerce_get_page_id('terms')) ); ?>" target="_blank"><?php _e( 'terms &amp; conditions', 'themeton' ); ?></a></label>
